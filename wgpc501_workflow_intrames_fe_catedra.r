...
DR_drifting_guantesblancos <- function( pmyexp, pinputexps, pserver="local")
{
  ...
  param_local$variables_iter1 <- FALSE
  param_local$variables_iter2 <- FALSE
...
ZZ_final_guantesblancos <- function( pmyexp, pinputexps, pserver="local")
{
  ...
  param_local$kaggle$envios_desde <-  8000L # ANTES 9500
  param_local$kaggle$envios_hasta <- 20000L # ANTES 11500
